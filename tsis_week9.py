...
while run:
    ...
    for x in range(0,800):
        y = int((height/2)+amp*math.sin(freq*((float(x)/width)*(2*math.pi))))
        pointz.append([x+150,y])
    pygame.draw.lines(screen, sin_color, False, pointz, 5)


    #cos line
    pointss = []
    for x in range(0,800,2):
        y = int((height/2) + amp*math.cos(freq*((float(x)/width)* (2*math.pi))))
        pointss.append([x+150,y])
    for i in range(0,len(pointss)-1,3):
        pygame.draw.line(screen, cos_color, (pointss[i][0],pointss[i][1]), (pointss[i+2][0],pointss[i+2][1]),3)
    ...
    screen.blit(font_for_bottom_panel.render('X', True, black),(565,690)) # X in the bottom
